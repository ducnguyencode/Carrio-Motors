...
@section('page-heading', 'Edit Model')
...
<div class="bg-white rounded-lg shadow-md p-8 max-w-3xl mx-auto">
    <form action="{{ route('admin.models.update', $model) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-1">
                ...
                <select name="make_id" id="make_id"
                    ...
                    @foreach ($makes as $make)
                        <option value="{{ $make->id }}" {{ old('make_id', $model->make_id) == $make->id ? 'selected' : '' }}>
                            {{ $make->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:outline-none @error('description') border-red-500 @enderror">{{ old('description', $model->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex justify-end gap-2 mt-8">
            ...
            <button type="submit" class="px-6 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 transition">Update Model</button>
        </div>
    </form>
</div>
@endsection
