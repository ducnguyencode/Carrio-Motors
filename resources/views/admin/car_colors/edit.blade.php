...
@section('title', 'Edit Car Color')

@section('page-heading', 'Edit Car Color')

@section('content')
<div class="bg-white rounded-lg shadow-md p-8 max-w-3xl mx-auto">
    <form action="{{ route('car_colors.update', $carColor) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="hex_code" id="hex_code"
                       value="{{ old('hex_code', $carColor->hex_code) }}"
            ...
            <div class="flex items-center mt-8">
                <input type="checkbox" name="is_active" id="is_active"
                       class="h-5 w-5 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" {{ old('is_active', $carColor->is_active) ? 'checked' : '' }}>
                <label for="is_active" class="ml-2 text-gray-700">Active</label>
            </div>
        </div>

        <div class="flex justify-end gap-2 mt-8">
            <a href="{{ route('car_colors.index') }}" class="px-5 py-2 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 font-medium transition">Cancel</a>
            <button type="submit" class="px-6 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 font-semibold shadow transition">Update Car Color</button>
        </div>
    </form>
</div>
@endsection